# Single-database setup: install Solid Queue / Cache / Cable tables into the
# primary DB so we don't need to provision separate databases on Render. The
# original Rails 8 default is one database per engine (db/{queue,cache,cable}_schema.rb)
# but on a single-Postgres host like Render's basic-1gb plan that's expensive
# and unnecessary. This migration consolidates them into one DB.

class InstallSolidEnginesIntoPrimaryDb < ActiveRecord::Migration[8.1]
  def change
    # ---- Solid Queue ----
